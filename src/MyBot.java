...
public class MyBot extends TelegramLongPollingBot {
    ...
    public void onUpdateReceived(Update update) {
        ...
        if (update.hasMessage() && update.getMessage().hasText()) {
            if (update.getMessage().getText().equals("/start")) {
                ...
                Main.initImage();
                if (Main.finalulr.toString().endsWith("jpg")) {
                    sendCustomKeyboard(update.getMessage().getChatId().toString(), 1, Main.finalulr.toString());
                }
                if (Main.finalulr.toString().endsWith("gif")) {
                    sendCustomKeyboard(update.getMessage().getChatId().toString(), 2, Main.finalulr.toString());
                }
    ...
    public void sendCustomKeyboard(String chatId, int code, String text) {
        ...
        switch (code) {
            ...
            case 1:
                sendImageFromUrl(Main.finalulr.toString(), chatId);
                break;
            case 2:
                try {
                    sendVideo(new SendVideo().setChatId(chatId).setVideo(Main.finalulr.toString()));
                    break;
                } catch (TelegramApiException e) {
                    e.printStackTrace();
                }
